/*
 * XPathVisitor.java : based on Sparta by Hewlett-Packard Company.
 * 
 ...
 * Visitor that evaluates an xpath expression relative to a context
 * node by walking over the parse tree of the expression.
 * @version  $Id: XPathVisitor.java 3385 2005-08-18 22:12:13Z go $
 * @author Go Takahashi
...
class XPathVisitor implements Visitor {
    ...
    private XPathVisitor(XPath xpath, Node context) /*throws XPathException*/ {
        ...
        for (Enumeration i = xpath.getSteps(); i.hasMoreElements();) {
            ...
            //nodesetIterator_ = nodelistRaw_.iterator();   // 2004.12.26 HAW-Ryan
			nodesetIterator_ = nodelistRaw_.vector_.elements();
    ...
    public void visit(ThisNodeTest a) {
		// 2004.12.26 HAW-Ryan modified
		//nodelistRaw_.removeAllElements();
		nodelistRaw_.vector_.removeAllElements();
		nodelistRaw_.positions_.clear();
		// End
            ...
            if (node instanceof Element) {
                ...
                for (Node n = element.getFirstChild();
                    ...
                    if (n instanceof Text)
                        //nodelistRaw_.add(((Text) n).getData());   // 2004.12.26 HAW-Ryan
				        nodelistRaw_.vector_.addElement( ((Text) n).getData() );
            }
        }
    }
    ...
    public void visit(AttrTest test) {
        ...
        for (Enumeration i = oldNodeList.elements(); i.hasMoreElements();) {
            ...
            if (node instanceof Element) {
                Element element = (Element) node;
                //String attr = element.getAttribute(test.getAttrName());    // comment by HAW-Ryan 2004.12.22
				String attr = element.getAttribute( null, test.getAttrName() );
                if (attr != null)
                    //nodelistRaw_.add(attr);    // 2004.12.26 HAW-Ryan commented
                    nodelistRaw_.vector_.addElement(attr);
            }
        }
    }
    ...
    public void visit(AttrExistsExpr a) /*throws XPathException*/ {
        ...
        //String attrValue = element.getAttribute(a.getAttrName());    // commented by HAW-Ryan 2004.12.22
		String attrValue = element.getAttribute( null, a.getAttrName()) ;
    ...
    public void visit(AttrLessExpr a) /*throws XPathException*/ {
        ...
        // commented by HAW-Ryan 2004.12.22
        //long attrValue = Long.parseLong(element.getAttribute(a.getAttrName()));
		long attrValue = Long.parseLong(element.getAttribute( null, a.getAttrName() ));
...
class NodeListWithPosition {
    // 2004.12.25 HAW-Ryan will not support
    ...
    // 2004.12.26 HAW-Ryan will not use
    ...
    void add(Node node, int position) {
		...
		Integer Int = new Integer(System.identityHashCode(node));
		positions_.put( Int, posn );
		// End
    }
    ...
    int position(Node node) {
		...
		Integer Int = new Integer(System.identityHashCode(node));
		return ((Integer) positions_.get(Int)).intValue();
		// End
    }
                ...
                if (e instanceof String)
                    ...
                    y.append(
                        "Node("
                            + n.toXml()
                            + ")["
                            //+ positions_.get(identity(n))   // replaced below
					        + positions_.get(new Integer(System.identityHashCode(n)))
                            + "] ");
                }
            }
            y.append("}");
	...
	// 2004.12.26 HAW-Ryan made vector and hashtable function public
	//private final Vector vector_ = new Vector();
    public final Vector vector_ = new Vector();
    //private Hashtable positions_ = new Hashtable();
	public Hashtable positions_ = new Hashtable();
}
...
// é¿ã@â“ìÆëŒâû
//
// Revision 1.2  2004/12/27 09:10:10  ryan
